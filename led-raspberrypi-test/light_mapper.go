...
func changeDeviceState(state string) {
	...
	glog.Info("Niu changeDeviceState()-> deviceStatusUpdate=DeviceETPrefix + deviceID + DeviceETStateUpdateSuffix:", deviceStatusUpdate)
...
func changeTwinValue(updateMessage DeviceTwinUpdate) {
	...
	glog.Info("Niu changeTwinValue()-> deviceTwinUpdate=DeviceETPrefix + deviceID + TwinETUpdateSuffix:", deviceTwinUpdate)
	if Token_client.Wait() && Token_client.Error() != nil {
...
func subscribe() {
	for {
		...
		glog.Info("Niu subscribe()-> getTwinResult = DeviceETPrefix + deviceID + TwinETGetResultSuffix:", getTwinResult)
		if Token_client.Wait() && Token_client.Error() != nil {
...
func main() {
	...
	for {
		...
		//loop every five seconds to test mqtt address: /twin/get
